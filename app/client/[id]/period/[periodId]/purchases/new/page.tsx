...
export default function NewPurchasePage({ params }: { params: { id: string; periodId: string } }) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            router.push(`/client/${params.id}/period/${params.periodId}/purchases`);
            router.refresh();
        } catch (err) {
        ...
        <div className="container" style={{ maxWidth: "800px" }}>
            ...
            <form onSubmit={handleSubmit} className="card" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--spacing-md)" }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                    <label style={{ display: "block", marginBottom: "var(--spacing-xs)", fontWeight: 500 }}>Proveedor (Razón Social)</label>
                    <input name="entityName" type="text" required className="input" />
                </div>

                <div>
                    <label style={{ display: "block", marginBottom: "var(--spacing-xs)", fontWeight: 500 }}>CUIT Proveedor</label>
